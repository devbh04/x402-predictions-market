...
interface OrderbookNoProps {
    noOrders: number[][]; // NO bids
    yesOrders: number[][]; // YES bids (used to derive NO asks)
    lastPrice?: number;
}
...
export default function OrderbookNo({ noOrders, yesOrders, lastPrice = 50 }: OrderbookNoProps) {
    const [selectedPrice, setSelectedPrice] = useState<number | null>(null);
    const asksScrollRef = useRef<HTMLDivElement>(null);

    // Process orderbook data
    const { asks, bids, maxContracts } = useMemo(() => {
        if (!noOrders && !yesOrders) {
            return { asks: [], bids: [], maxContracts: 0 };
        }

        // NO bids come directly from noOrders
        const noBids: OrderLevel[] = (noOrders || []).map(([price, contracts]) => ({
            price,
            contracts,
            total: (price * contracts) / 100,
        }));

        // NO asks are derived from YES bids: 100 - YES bid price
        const noAsks: OrderLevel[] = (yesOrders || []).map(([price, contracts]) => ({
            price: 100 - price,
            contracts,
        ...
        const asks = noAsks.sort((a, b) => b.price - a.price);
        ...
        <div className="space-y-2">
            ...
            <div className="relative py-3">
                ...
                <div className="relative flex items-center justify-center gap-2 bg-zinc-900 px-3 mx-auto w-fit">
                    <span className="text-blue-400 font-semibold text-sm">Trade No</span>
